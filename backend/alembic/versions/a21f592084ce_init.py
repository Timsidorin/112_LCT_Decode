...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "title", sa.String(length=150), nullable=False, comment="Название курса"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["publication_id"], ["training_publications.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_training_passage_attempts_publication_id"),
        "training_passage_attempts",
        ["publication_id"],
        unique=False,
    )
